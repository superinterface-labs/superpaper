...
- **Claim** — an assertion that could be wrong. Has `confidence` + evidence links. Subsumes hypotheses and models — if it explains something, it's a claim about how it works.
- **Analogy** — a deep structural parallel between distant domains. Maps *why* the parallel holds, what transfers, and where it breaks. Subsumes patterns and bridges — if it connects domains, it's an analogy.
...
- **Preference** — how someone thinks, works, or wants things done. Lives in `meta/`. Preferences take precedence over general heuristics.
...
2. **Decision made** — capture the *reasoning*, not just the outcome (use `type: decision`)
...
7. **Connection discovered** — two previously unlinked concepts share structural similarity. Create an analogy note.
...
26 starter category pages ship in `superpaper/categories/` and base templates in `_templates/Bases/`. When a new domain emerges, spin up the trinity: copy a base template, create a category page, optionally add a note template. Instant.
...
4. **Extract insights** — pull key claims and ideas into atomic knowledge notes in `concepts/`. Every extracted note MUST link back to the bookmark file (`[[bookmark-title]]`) so the base views can surface it via `file.link`.
...
3. Promote high-signal claims, concepts, and analogies into `superpaper/concepts/`
...
**Elegant simplicity.** Entity folders are broad enough to last forever. Subfolders within them emerge only when volume demands it — never before. A clean vault invites use; a pre-organized one intimidates. But if the human prefers a different layout — fewer folders, different names, flat structure — go with it.
...
`AGENTS.md` exists in exactly three places:

1. **Root** — this file. The vault OS.
2. **`.agents/skills/AGENTS.md`** — skill index.
3. **`_templates/AGENTS.md`** — template conventions.

Each has a `CLAUDE.md` symlink (`ln -sf AGENTS.md CLAUDE.md`) so Claude Code discovers it. No other folders need `AGENTS.md` files — the skill system and templates handle context.
...
- **Infrastructure** — defines how the OS works. Distributable, versioned, shared: `AGENTS.md` (root, `.agents/skills/`, `_templates/`), `.agents/**`, `_templates/**`, `superpaper/categories/**`, `obsidian-types-init.json`, `.obsidian/**`, `.scripts/**`.
...
This step is critical — the vault needs plugins to function well. **Configure ALL plugins — both core and community.** Don't skip core plugin settings; defaults are wrong for Superpaper.

**Core settings:** Settings → Files & Links → enable **Automatically update internal links** and set **Default location for new attachments** to `_attachments/`.

**Enable core plugins:** Settings → Core plugins → turn on everything except **Random note** and **Publish**. This ensures Bases, Properties, Backlinks, Outgoing links, Tags, Templates, Word count, Daily notes, and all other native features are available.

**Configure core plugins** (non-default settings required):
...
**Deploy `Daily.base`:** Copy `_templates/Bases/Daily.base` to `daily/Daily.base`. This is the dashboard embedded on every daily note — its default "Everything" view shows all notes linked to that date except pure AI-generated ones.
...
**Verify with human.** Ask: "Can you see any `AGENTS.md` files in your file explorer?" If yes, debug the hide filters. Confirm `_templates` and `inbox` are hidden. Confirm plugins are working. **The human's visual confirmation is the only proof.**
